...
import ch.ehi.sqlgen.generator_impl.jdbc.GeneratorJdbc.AbstractStmt;
...
public class Ili2db {
		...
		try {
			...
			if (function == Config.FC_DELETE) {
				...
				if (ch.ehi.basics.view.GenericFileFilter.getFileExtension(inputFilename).toLowerCase().equals("zip")) {
					try {
						...
						Enumeration<? extends ZipEntry> filei = zipFile.entries();
						
						while (filei.hasMoreElements()) {
							java.util.zip.ZipEntry zipEntry = (java.util.zip.ZipEntry) filei.nextElement();
							String ext = ch.ehi.basics.view.GenericFileFilter.getFileExtension(zipEntry.getName())
									.toLowerCase();
			...
			if (function != Config.FC_DELETE) {
				...
				for (int modeli = 0; modeli < modelnames.length; modeli++) {
					...
					if (m != null) {
						modelv.addFileEntry(new ch.interlis.ili2c.config.FileEntry(m,
			...
			try {
					...
					if (datasetName != null) {
						...
						if (!createBasketCol) {
							throw new Ili2dbException(
									"import with dataset name requires column " + DbNames.T_BASKET_COL);
						}
					}
				}
				...
				TransferDescription td = ch.interlis.ili2c.Main.runCompiler(modelv, config);
				if (td == null) {
				...
				java.util.List<Element> eles = ms.getModelElements(modelNames, td,
						td.getIli1Format() != null && config.getDoItfLineTables(),
						Config.CREATE_ENUM_DEFS_MULTI.equals(config.getCreateEnumDefs()));
				Viewable2TableMapping class2wrapper = Viewable2TableMapper.getClass2TableMapping(config, trafoConfig,
						eles, mapping);
				...
				if (zipXtfEntry != null) {
					...
					try {
						...
						transferFromXtf(conn, ioxReader, function, mapping, td, dbusr, geomConverter, idGen, config,
								stat, trafoConfig, class2wrapper);
					} catch (IOException ex) {
						throw new Ili2dbException(ex);
					...
					if (attachmentsBase != null) {
						...
						while (filei.hasMoreElements()) {
							...
							if (!zipXtfEntry.getName().equals(zipEntry.getName())) {
								...
								if (!parent.exists()) {
									if (!parent.mkdirs()) {
										throwIli2dbException("failed to create: " + parent.getAbsolutePath());
									}
								}
								try {
									copyStream(destFile, zipFile.getInputStream(zipEntry));
								} catch (IOException ex) {
									throwIli2dbException("failed to save attachment " + zipEntry.getName(), ex);
								}
							}
						}
							...
							if (isItfFilename(inputFilename)) {
								if (config.getDoItfLineTables()) {
									...
									ioxReader = new ItfReader2(new java.io.File(inputFilename),
											config.ignorePolygonBuildingErrors());
									((ItfReader2) ioxReader).setModel(td);
								}
							} else {
	...
	private static void throwIli2dbException(String error, IOException ex) throws Ili2dbException {
		throw new Ili2dbException(error, ex);
	}

	private static void throwIli2dbException(String error) throws Ili2dbException {
		throw new Ili2dbException(error);
	}
	...
	private static void logStatistics(boolean isIli1, HashSet<BasketStat> stat) {
		...
		for (BasketStat basketStat : statv) {
			if (isIli1) {
				...
				EhiLogger.logState(
						basketStat.getFile() + ": " + basketStat.getTopic() + " BID=" + basketStat.getBasketId());
			}
		...
		try {
			...
			try {
				...
				if (xtffile != null && xtffile.endsWith(".ili")) {
					ilifile = xtffile;
					modelv.addFileEntry(new ch.interlis.ili2c.config.FileEntry(ilifile,
							ch.interlis.ili2c.config.FileEntryKind.ILIMODELFILE));
				}
				...
				modelv.setAutoCompleteModelList(true);
				modelv.setGenerateWarnings(false);
				td = ch.interlis.ili2c.Main.runCompiler(modelv, config);
				if (td == null) {
					throw new Ili2dbException("compiler failed");
				}
				...
				try {
					...
					// if(conn instanceof
					// ch.ehi.ili2geodb.jdbc.GeodbConnection){
					// String xmlfile=null;
					// try{
					// com.esri.arcgis.geodatabasedistributed.GdbExporter
					// exp=new
					// com.esri.arcgis.geodatabasedistributed.GdbExporter();
					// xmlfile=config.getDbfile()+".xml";
					// exp.exportWorkspaceSchema(((ch.ehi.ili2geodb.jdbc.GeodbConnection)conn).getGeodbWorkspace(),
					// xmlfile, false,false);
					// }catch(Throwable ex){
					// EhiLogger.logError("failed to export gdb to
					// "+xmlfile,ex);
					// }
					// }
					if (!connectionFromExtern) {
	...
	private static void setupIli2cPathmap(Config config, String appHome, String xtffile, java.sql.Connection conn)
		...
		if (conn != null) {
			...
			if (iliFiles != null) {
				...
				config.setTransientObject(ch.interlis.ili2c.gui.UserSettings.CUSTOM_ILI_RESOLVER,
	...
	public static void runExport(Config config, String appHome) throws Ili2dbException {
		...
		try {
			...
			try {
				...
				EhiLogger.getInstance().addListener(errs);
				transferToXtf(conn, xtffile, mapping, td, geomConverter, config.getSender(), config, exportModelnames,
						basketSqlIds, stat, trafoConfig, class2wrapper);
				if (errs.hasSeenErrors()) {
	...
	private static Connection connect(String url, String dbusr, String dbpwd, Config config,
			CustomMapping customMapping) throws SQLException {
		Connection conn;
	...
	public static long[] getBasketSqlIdsFromDatasetId(long datasetId, Configuration modelv, Connection conn,
			Config config) throws Ili2dbException {
		...
		java.sql.PreparedStatement getstmt = null;
		try {
			String stmt = "SELECT " + colT_ID + "," + DbNames.BASKETS_TAB_TOPIC_COL + " FROM " + sqlName + " WHERE "
			...
			while (res.next()) {
				...
				if (!models.contains(modelName)) {
					modelv.addFileEntry(new ch.interlis.ili2c.config.FileEntry(modelName,
							ch.interlis.ili2c.config.FileEntryKind.ILIMODELFILE));
	...
	private static long[] getBasketSqlIdsFromBID(String[] basketids, Configuration modelv, Connection conn,
			Config config) throws Ili2dbException {
	...
	public static Long getBasketSqlIdFromBID(String basketid, Connection conn, String schema, String colT_ID,
			StringBuilder topicName) throws Ili2dbException {
	...
	private static long[] getBasketSqlIdsFromTopic(String[] topics, Configuration modelv, Connection conn,
			Config config) throws Ili2dbException {
		String schema = config.getDbschema();
		String colT_ID = config.getColT_ID();
		if (colT_ID == null) {
			...
			while (res.next()) {
				...
				for (String qryTopic[] : qryTopics) {
					if (qryTopic[0] == null && qryTopic[1].equals(dbTopic[1])) {
						...
						modelName = dbTopic[0];
						break;
					} else if (qryTopic[0] != null && qryTopic[0].equals(dbTopic[0])
							&& qryTopic[1].equals(dbTopic[1])) {
						// found one
						modelName = dbTopic[0];
						break;
					}
	...
	private static long[] getBasketSqlIdsFromModel(String[] qryModels, Configuration modelv, Connection conn,
			Config config) throws Ili2dbException {
		String schema = config.getDbschema();
		String colT_ID = config.getColT_ID();
		if (colT_ID == null) {
	...
	public static ArrayList<String> getModelDirv(String modeldir, String xtffile, String ili2dbHome) {
	...
	static private void transferFromXtf(Connection conn, IoxReader reader, int function, NameMapping ili2sqlName,
			TransferDescription td, String dbusr, SqlColumnConverter geomConv, DbIdGen idGen, Config config,
			HashSet<BasketStat> stat, TrafoConfig trafoConfig, Viewable2TableMapping class2wrapper) {
		try {
			TransferFromXtf trsfr = new TransferFromXtf(function, ili2sqlName, td, conn, dbusr, geomConv, idGen, config,
					trafoConfig, class2wrapper);
	...
	static private void transferToXtf(Connection conn, String xtffile, NameMapping ili2sqlName, TransferDescription td,
			SqlColumnConverter geomConv, String sender, Config config, String exportParamModelnames[],
			long basketSqlIds[], HashSet<BasketStat> stat, TrafoConfig trafoConfig,
	...
	static private HashSet<String> getModelNames(ArrayList<String> modelv) {
		HashSet<String> ret = new HashSet<String>();
		Iterator<String> modeli = modelv.iterator();
		while (modeli.hasNext()) {
			String topic = modeli.next(); // modelname or qualified
													// topicname
	...
	static private void writeScript(String filename, Iterator linei) throws java.io.IOException {
		java.io.PrintWriter out = new java.io.PrintWriter(
				new java.io.BufferedOutputStream(new java.io.FileOutputStream(filename)));
		while (linei.hasNext()) {
			GeneratorJdbc.AbstractStmt stmt = (AbstractStmt) linei.next();
	...
	public static void logDBVersion(Connection conn) {
		try {
			EhiLogger.logState("databaseProduct <" + conn.getMetaData().getDatabaseProductName() + ">");
			EhiLogger.logState("databaseVersion <" + conn.getMetaData().getDatabaseProductVersion() + ">");
			EhiLogger.logState("driverVersion <" + conn.getMetaData().getDriverVersion() + ">");
