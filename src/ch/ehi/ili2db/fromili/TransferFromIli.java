...
public class TransferFromIli {
	...
	private HashSet<AbstractLeafElement> visitedEnums = null;
	...
	public DbSchema doit(TransferDescription td1, java.util.List<Element> modelEles,
			ch.ehi.ili2db.mapping.NameMapping ili2sqlName, ch.ehi.ili2db.gui.Config config, DbIdGen idGen,
			TrafoConfig trafoConfig, Viewable2TableMapping class2wrapper1, CustomMapping customMapping1)
		...
		visitedEnums = new HashSet<AbstractLeafElement>();
		td = td1;
	...
	private void generatModelEles(java.util.List<Element> modelEles, int pass) throws Ili2dbException {
		Iterator<Element> modeli = modelEles.iterator();
		while (modeli.hasNext()) {
			...
			// }else
			if (modelo instanceof Domain && pass == 2) {
				...
				if (modelo instanceof Table && ((Table) modelo).isIli1LineAttrStruct()) {
					...
					try {
						...
						throw new Ili2dbException("mapping of " + ((Viewable<?>) modelo).getScopedName(null) + " failed",
								ex);
					}
				}
	...
	private void generateItfLineTable(AttributeDef attr, int pass) throws Ili2dbException {
		...
		DbColGeometry dbCol = recConv.generatePolylineType(type,
				attr.getContainer().getScopedName(null) + "." + attr.getName());
	...
	/**
	 * setup a mapping from a qualified model or topic name to the corresponding
	 * java object.
	...
	public static ch.interlis.ilirepository.IliFiles readIliFiles(java.sql.Connection conn, String schema)
		...
		try {
			...
			// select entries
			String insStmt = "SELECT " + DbNames.MODELS_TAB_FILE_COL + "," + DbNames.MODELS_TAB_ILIVERSION_COL + ","
					+ DbNames.MODELS_TAB_MODELNAME_COL + " FROM " + sqlName;
			...
			try {
				...
				while (rs.next()) {
					...
					ch.interlis.ili2c.modelscan.IliFile iliFile = IliImportsUtility.parseIliImports(iliversion,
							imports);
	...
	public static String readIliFile(java.sql.Connection conn, String schema, String filename) throws Ili2dbException {
		...
		try {
			// select entries
			String selStmt = "SELECT " + DbNames.MODELS_TAB_CONTENT_COL + " FROM " + sqlName + " WHERE "
					+ DbNames.MODELS_TAB_FILE_COL + "=?";
	...
	public static void addModels(java.sql.Connection conn, TransferDescription td, String schema)
		...
		try {
			...
			// insert entries
			String insStmt = "INSERT INTO " + sqlName + " (" + DbNames.MODELS_TAB_FILE_COL + ","
					+ DbNames.MODELS_TAB_ILIVERSION_COL + "," + DbNames.MODELS_TAB_MODELNAME_COL + ","
					+ DbNames.MODELS_TAB_CONTENT_COL + "," + DbNames.MODELS_TAB_IMPORTDATE_COL + ") VALUES (?,?,?,?,?)";
			EhiLogger.traceBackendCmd(insStmt);
			java.sql.PreparedStatement insPrepStmt = conn.prepareStatement(insStmt);
			Iterator<Model> entri = td.iterator();
			...
			try {
				for (ch.interlis.ili2c.modelscan.IliFile ilifile : ilifiles.values()) {
					ch.interlis.ili2c.modelscan.IliModel ilimodel = (ch.interlis.ili2c.modelscan.IliModel) ilifile
							.iteratorModel().next();
					if (iliModelsInDb == null
							|| iliModelsInDb.getFileWithModel(ilimodel.getName(), ilimodel.getIliVersion()) == null) {
	...
	public static String readFileAsString(java.io.File filePath) throws java.io.IOException {
		...
		try {
			...
			if (len > Integer.MAX_VALUE)
				throw new java.io.IOException("File " + filePath + " too large, was " + len + " bytes.");
	...
	public static void readSettings(java.sql.Connection conn, Config settings, String schema) throws Ili2dbException {
		...
		if (DbUtility.tableExists(conn, new DbTableName(schema, DbNames.SETTINGS_TAB))) {
			try {
				// select entries
				String insStmt = "SELECT " + DbNames.SETTINGS_TAB_TAG_COL + "," + DbNames.SETTINGS_TAB_SETTING_COL
						+ " FROM " + sqlName;
				EhiLogger.traceBackendCmd(insStmt);
				java.sql.PreparedStatement insPrepStmt = conn.prepareStatement(insStmt);
	...
	public static void updateSettings(java.sql.Connection conn, Config settings, String schema) throws Ili2dbException {
		...
		try {
			...
			// insert entries
			String insStmt = "INSERT INTO " + sqlName + " (" + DbNames.SETTINGS_TAB_TAG_COL + ","
					+ DbNames.SETTINGS_TAB_SETTING_COL + ") VALUES (?,?)";
			EhiLogger.traceBackendCmd(insStmt);
			java.sql.PreparedStatement insPrepStmt = conn.prepareStatement(insStmt);
			try {
				Iterator<String> entri = settings.getValues().iterator();
				while (entri.hasNext()) {
	...
	public void addImportsTable(DbSchema schema) {
		{
			...
			dbColFile.setNotNull(false); // NULLable so that delete can only be
											// logged
	...
	public void addEnumTable(DbSchema schema) {
		if (Config.CREATE_ENUM_DEFS_SINGLE.equals(createEnumTable)) {
			...
			Iterator<AbstractLeafElement> entri = visitedEnums.iterator();
			while (entri.hasNext()) {
				Object entro = entri.next();
	...
	public void updateInheritanceTable(java.sql.Connection conn, String schema) throws Ili2dbException {
		...
		// String stmt="CREATE TABLE "+tabname+" ("+thisClassCol+" VARCHAR2(30)
		// NOT NULL,"+baseClassCol+" VARCHAR2(30) NULL)";
		HashSet<String> exstEntries = readInheritanceTable(conn, schema);
		try {

			// insert entries
			String stmt = "INSERT INTO " + sqlName + " (" + DbNames.INHERIT_TAB_THIS_COL + ","
					+ DbNames.INHERIT_TAB_BASE_COL + ") VALUES (?,?)";
	...
	public void updateEnumTable(java.sql.Connection conn) throws Ili2dbException {
		if (Config.CREATE_ENUM_DEFS_SINGLE.equals(createEnumTable)) {
	...
	private static HashSet<String> readEnumTable(java.sql.Connection conn, boolean singleTable, String qualifiedIliName,
			DbTableName sqlDbName) throws Ili2dbException {
		HashSet<String> ret = new HashSet<String>();
		...
		try {
			...
			if (!singleTable) {
				...
				exstStmt = "SELECT " + DbNames.ENUM_TAB_ILICODE_COL + " FROM " + sqlName + " WHERE "
						+ DbNames.ENUM_TAB_THIS_COL + " = '" + qualifiedIliName + "'";
			}
			EhiLogger.traceBackendCmd(exstStmt);
			java.sql.PreparedStatement exstPrepStmt = conn.prepareStatement(exstStmt);
			try {
	...
	public void updateSingleEnumTable(java.sql.Connection conn) throws Ili2dbException {
		...
		try {
			...
			// insert entries
			String insStmt = "INSERT INTO " + sqlName + " (" + DbNames.ENUM_TAB_SEQ_COL + ","
					+ DbNames.ENUM_TAB_ILICODE_COL + "," + DbNames.ENUM_TAB_ITFCODE_COL + ","
					+ DbNames.ENUM_TAB_DISPNAME_COL + "," + DbNames.ENUM_TAB_INACTIVE_COL + ","
					+ DbNames.ENUM_TAB_THIS_COL + "," + DbNames.ENUM_TAB_BASE_COL + ") VALUES (?,?,?,?,?,?,?)";
			EhiLogger.traceBackendCmd(insStmt);
			java.sql.PreparedStatement insPrepStmt = conn.prepareStatement(insStmt);
			String thisClass = null;
			try {
		...
		while (entri.hasNext()) {
			...
			if (entro instanceof AttributeDef) {
				...
				try {
					...
					// insert entries
					String stmt = "INSERT INTO " + thisSqlName + " (" + DbNames.ENUM_TAB_SEQ_COL + ","
							+ DbNames.ENUM_TAB_ILICODE_COL + "," + DbNames.ENUM_TAB_ITFCODE_COL + ","
							+ DbNames.ENUM_TAB_DISPNAME_COL + "," + DbNames.ENUM_TAB_INACTIVE_COL
							+ ") VALUES (?,?,?,?,?)";
					EhiLogger.traceBackendCmd(stmt);
	...
	private void updateEnumEntries(HashSet<?> exstEntires, java.sql.PreparedStatement ps, EnumerationType type,
			String thisClass, String baseClass) throws SQLException {
		...
		int itfCode = 0;
		int seq = 0;
		Iterator<String> evi = ev.iterator();
		while (evi.hasNext()) {
